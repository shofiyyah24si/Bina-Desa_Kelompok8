...
/**
 * Script untuk memperbaiki semua database dan tabel agar upload foto berfungsi
...
echo "<h1>🔧 Perbaiki Database Lengkap</h1>";
...
try {
    ...
    try {
        ...
        if (!in_array('foto', $wargaColumns)) {
            $pdo->exec("ALTER TABLE warga ADD COLUMN `foto` VARCHAR(255) NULL");
    ...
    // 4. Cek dan buat tabel-tabel yang diperlukan
    echo "<h2>4. Memastikan Semua Tabel Ada</h2>";
    ...
    // 6. Update semua user jadi Admin
    echo "<h2>6. Update Semua User Jadi Admin</h2>";
    try {
        $stmt = $pdo->prepare("UPDATE users SET role = 'Admin' WHERE role IS NULL OR role = '' OR role = 'User'");
        $result = $stmt->execute();
        $affected = $stmt->rowCount();
        echo "<p style='color: green;'>✅ $affected user diupdate menjadi Admin</p>";
    } catch (Exception $e) {
        echo "<p style='color: orange;'>⚠️ Update role: " . $e->getMessage() . "</p>";
    }
    
    // 7. Tampilkan ringkasan tabel
    echo "<h2>7. Ringkasan Tabel</h2>";
    ...
    echo "<h2>🎉 Perbaikan Database Lengkap Selesai!</h2>";
    echo "<div style='background: white; padding: 15px; border-radius: 5px; margin: 10px 0;'>";
    echo "<p><strong>Yang sudah diperbaiki:</strong></p>";
    echo "<ol>";
    echo "<li>✅ Tabel media dibuat</li>";
    echo "<li>✅ Tabel users diperbaiki (foto_profil, role, last_login_at)</li>";
    echo "<li>✅ Tabel warga diperbaiki (foto)</li>";
    echo "<li>✅ Semua tabel utama sudah ada</li>";
    echo "<li>✅ Folder upload sudah dibuat</li>";
    echo "<li>✅ Semua user dijadikan Admin</li>";
    echo "</ol>";
    
    echo "<p><strong>Langkah selanjutnya:</strong></p>";
    echo "<ol>";
    echo "<li>🔄 Upload semua controller yang sudah diperbaiki</li>";
    echo "<li>🧪 Test semua halaman (users, warga, kejadian, dll)</li>";
